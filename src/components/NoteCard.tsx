...
export function NoteCard({ note, showImage = false }: NoteCardProps) {
  return (
    <Link
      href={`/work/${note.slug}/`}
      className="group flex gap-4 border-t border-rule py-6 transition-transform duration-300 hover:translate-x-1 sm:gap-6"
    >
      {showImage && note.image ? (
        <ProjectImage src={note.image} alt="" variant="thumb" />
      ) : null}
